...
async def debug_analytics(request: Request):
    ...
    from app.services import analytics_svc, trade_svc
    ...
    async for db in get_db():
        ...
        try:
            ...
            direct_ok = True
            direct_err = None
            direct_trades = data.get("overview", {}).get("total_trades")
        except Exception as e:
            direct_ok = False
            direct_err = traceback.format_exc()
            direct_trades = None

        # Through service (same as /analytics/full)
        try:
            user_result = await db.execute(select(User).limit(1))
            user = user_result.scalar_one_or_none()
            if user:
                user_trades = await trade_svc.get_user_trades(db, user, limit=10000)
                svc_data = await analytics_svc.get_full_analytics(user_trades, user, 0)
                svc_ok = True
                svc_err = None
